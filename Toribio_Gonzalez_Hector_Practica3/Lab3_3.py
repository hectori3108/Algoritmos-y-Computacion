...
def DFSListaEtiquCiclos(E, V, i, fil, col, cuelgade):               #EN ESTE CASO INICIALIZAMOS CUELGADE FUERA DEL CICLO YA QUE VAMOS A IR ACTUALIZANDOLA CON CADA ITERACION
    stack = []
...
def BFSListaEtiquCiclos(E, V, i, fil, col, cuelgade):                   #EN ESTE CASO INICIALIZAMOS CUELGADE FUERA DEL CICLO YA QUE VAMOS A IR ACTUALIZANDOLA CON CADA ITERACION
    stack = []
...
def recorreListaEtiquCiclos(cuelgade, V, E, fil, col):
    matrizPinta=[]
    ...
    for k in range(len(V)):
        if(V[k].compVis()):
            ...
            plt.text((int(columna(V[k].compNum(),c)))*2+1,int((fila(V[k].compNum(),c)))*2+1,V[k].compNiv(),fontsize=8,color='yellow',verticalalignment='center',horizontalalignment='center')
    ...
    #RECORREMOS DE NUEVO LA MATRIZ Y EN LAS CASILLAS EN LA QUE HAYA UN 10, SIGNIFICA QUE HAY UN CICLO E IMPRIMIMOS CC
    for i in range(fil*2+1):
        for j in range(col*2+1):
            if(matrizPinta[i][j] == 10):
                plt.text(j,i,"cc",fontsize=8,color='red',verticalalignment='center',horizontalalignment='center')
    return matrizPinta
...
#SI NO SE HA RECORRIDO HACEMOS BUSQUEDA DESDE ESE NODO
#SI YA SE HA RECORRIDO PASAMOS AL SIGUIENTE
#ESTE ESTA HECHO PARA PROFUNDIDAD
def ciclosRepProf(V, asociaciones, E, F, C):
    for j in range(0, len(V)-1):
        if(V[j].compVis() == False):
            asociaciones = DFSListaEtiquCiclos(E, V, j, f, c, asociaciones)

#ESTA ES LA PARTE IMPORTANTE DE ESTA MODIFICACION, RECORREMOS LA MATRIZ ENTERA MIRANDO SI EL NODO EN EL QUE ESTAMOS SE HA RECORRIDO
#SI NO SE HA RECORRIDO HACEMOS BUSQUEDA DESDE ESE NODO
#SI YA SE HA RECORRIDO PASAMOS AL SIGUIENTE
#ESTE ESTA HECHO PARA ANCHURA
def ciclosRepAnch(V, asociaciones, E, F, C):
    for j in range(0, len(V)-1):
        if(V[j].compVis() == False):
            asociaciones = BFSListaEtiquCiclos(E, V, j, f, c, asociaciones)
...
if(anprof == 'P'):
    ...
    ciclosRepProf(V, asociaciones, E, f, c)
    timeFin = time.time()
else:
    timeIn = time.time()
    #LLAMAMOS AL METODO QUE RECORRE TODA LA MATRIZ
    ciclosRepAnch(V, asociaciones, E, f, c)
    timeFin = time.time()
matrizpinta = recorreListaEtiquCiclos(asociaciones, V, E, f, c)
